...
const Sidebar = ({ isOpen, onToggle, isMobile }) => {
  ...
  className={`fixed inset-0 bg-black/30 z-[950] ${
    isMobile && isOpen ? "block" : "hidden"
      ...
      <aside
        className={`
          fixed top-0 left-0 bottom-0 z-1000 bg-white border-r border-slate-100 flex flex-col transition-all duration-300 overflow-visible
          ${
            isMobile
              ? `w-[285px] ${isOpen ? "translate-x-0" : "-translate-x-full"}`
              : isOpen
                ? "w-[285px]"
                : "w-[82px]"
          }
